...
// FileBackend reads SecretSync configurations from YAML files
...
func (b *FileBackend) loadConfigFile(path string) error {
	...
	for _, doc := range docs {
		...
		var sync v1alpha1.SecretSync
		if err := yaml.Unmarshal([]byte(doc), &sync); err != nil {
			l.WithError(err).Debug("failed to unmarshal as SecretSync, trying raw spec")
			...
			if kind, ok := rawConfig["kind"].(string); ok && kind == "SecretSync" {
				// Re-parse with proper structure
				if err := yaml.Unmarshal([]byte(doc), &sync); err != nil {
					return fmt.Errorf("failed to unmarshal SecretSync: %w", err)
				}
			} else {
				continue // Skip non-SecretSync documents
			}
		}
...
// LoadFromPipelineConfig loads SecretSync configs generated by the pipeline
func (b *FileBackend) LoadFromPipelineConfig(configs []v1alpha1.SecretSync) error {
	l := log.WithFields(log.Fields{
...
func GetAllConfigs() []v1alpha1.SecretSync {
	configs := make([]v1alpha1.SecretSync, 0, len(SyncConfigs))
